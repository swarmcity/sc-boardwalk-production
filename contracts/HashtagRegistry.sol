...
contract HashtagRegistry is Ownable {

    mapping(address=>bytes32) public hashtags;

    function HashtagRegistry(){
    }

    function newHashtag(address _hashtag, bytes32 _ipfsHash) {
        IHashtag hashtag = IHashtag(_hashtag);
        if (msg.sender != hashtag.getConflictResolver()) {
            throw;
        }

        hashtags[msg.sender] = _ipfsHash;
